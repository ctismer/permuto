...
The original's two modes are flags: `--pg` for the permutograph mode (which is
also what no arguments does) and `--iridium` for SIMONE. `permuto <command>
--help` explains each command.
