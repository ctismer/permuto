#
# 02.05.92:
# slight modification to get the original unmapped node labels:
# iff parameter 2 existing.
# Changed numbering to the use of character strings that show up nicely.
#
#
...
function make_key(a, b, c, na, nb, nc    ,res) {
  a = substr("abcdefghijkl", a, 1)
  a = a a ; a = a a ; a = a a ; a = a a   # 16 max
  b = substr("abcdefghijkl", b, 1)
  b = b b ; b = b b ; b = b b ; b = b b   # 16 max
  c = substr("abcdefghijkl", c, 1)
  c = c c ; c = c c ; c = c c ; c = c c   # 16 max
  if (na > 0) res = res "+" substr(a, 1, na)
  if (nb > 0) res = res "+" substr(b, 1, nb)
  if (nc > 0) res = res "+" substr(c, 1, nc)
  return substr(res,2)
}
...
BEGIN {
  ...
  for (tr in Tri) {
    ...
    for (i in Edges) {                      # Kanten ausgeben und putzen
      ...
      if (DOMAP) print Map[x[1]], Map[x[2]]
      else       print pretty(x[1]), pretty(x[2])
    }
